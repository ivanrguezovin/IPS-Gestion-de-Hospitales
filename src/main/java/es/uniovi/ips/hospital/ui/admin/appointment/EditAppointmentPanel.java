...
public class EditAppointmentPanel extends JPanel implements Shiftable {
	...
	public void fillComboBoxes() {
		...
		if (autoCompleteSelectedDoctor == null)
			autoCompleteSelectedDoctor = AutoCompleteSupport.install(getCbSelectedDoctors(), selectedDoctorsList,
					new StaffTextFilterator(), staffFormat);
		...
		if (autoCompleteSelectedNurse == null)
			autoCompleteSelectedNurse = AutoCompleteSupport.install(getCbSelectedNurses(), selectedNursesList,
					new StaffTextFilterator(), staffFormat);
	...
	public void setAppointment(Appointment appointment) {
		...
		calendar.setDate(Date.from(
				appointment.getStartTime().toLocalDate().atStartOfDay().atZone(ZoneId.systemDefault()).toInstant()));
		...
		lblDoctors.setText("<html><p style=\"width:500px\">"
				+ selectedDoctors.stream().map(d -> d.guiToString()).collect(Collectors.joining(", ")) + "<p></html>");
		...
		lblNurses.setText("<html><p style=\"width:500px\">"
				+ selectedNurses.stream().map(n -> n.guiToString()).collect(Collectors.joining(", ")) + "<p></html>");
		setDate();
	}
	...
	private void modifyAppointment() {
		try {
			...
			if (JOptionPane.showConfirmDialog(this,
					"This will modify the selected appointment, are you sure?") != JOptionPane.YES_OPTION)
				return;
			if (isThereUnavailableDoctor())
				if (JOptionPane.showConfirmDialog(this,
						"One of the selected doctors doesn't work on the specified time, are you sure?") != JOptionPane.YES_OPTION)
					return;
			// Creación de la cita
	...
	// Añade el enfermero a la lista de enfermeros de la cita y lo sustrae de los
	// posibles
	...
	// Elimina el doctor de la lista de doctores de la cita y lo añade a la de los
	// posibles doctores
